...
var serviceName = 'pluginTapestry';
...
exports.register = function( plugin, options, next ) {
	...
	*/
	function makeItSo( options ) {
		...
		try {
			...
			if( !options.ids ) {
				throw new TypeError( 'invalid options.ids' );
			}
			...
			tapestry.get( { ids: options.ids, identifier: options.identifier }, function( error, result ) {
				//console.log('er', error, result);
				// Add the returned result as a content object under the original detected node.
				if( !_.isUndefined( result ) && result.length > 0 ) {
					console.log( 'result' );
					...
					deferred.reject( {
						error: 'no results returned from Tapestry',
	...
	*/
	...
	plugin.expose( 'makeItSoComplex', makeItSoComplex );
