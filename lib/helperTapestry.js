...
module.exports = {
	do: function( deferred, options ) {
		...
		// TODO: fire up our loggly-interface and redis-cache-wrapper
		var RedisCacheWrapper = require( 'redis-cache-wrapper' );
		var redisCacheWrapper = new RedisCacheWrapper( {
			// host: "revolver.6q2jla.0001.euw1.cache.amazonaws.com",
			// port: "11211"
			host: "127.0.0.1",
			port: "6379"
		} );

		// add the cache configuration to the global tapestry configuration
		_.assign( options.config, {
			cache: {
				client: redisCacheWrapper,
				namespace: 'theWorks'
			}
		} );

		// Create a new Tapestry object based on the passed in configuration
		var tapestry = new Tapestry( options.config );
		...
		*/
		...
		extractCodesAndCallTapestry( options.inputData, options.codePatterns );
